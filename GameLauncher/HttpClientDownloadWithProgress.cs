...
class HttpClientDownloadWithProgress : IDisposable
{
    ...
    public delegate void OnFileDownloaded(string destinationFilePath);

    public event ProgressChangedHandler ProgressChanged;
    public event OnFileDownloaded FileDownloaded;
    ...
    private void TriggerProgressChanged(long? totalDownloadSize, long totalBytesRead, bool hasDisposed)
    {
        ...
        if (progressPercentage.HasValue && progressPercentage.Value == 100 && hasDisposed)
        {
            FileDownloaded(_destinationFilePath);
        }
    }
